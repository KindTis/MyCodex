...
공식 구현 계획 문서가 공식 스펙 문서의 요구사항을 빠짐없이 충족하고, 구현자가 바로 작업할 수 있을 정도로 명확한지 검토하고 개선한다.

대상은 게임 구현 문서가 아니라 Codex 사용량 로컬 웹 대시보드 구현 계획이다. 하위 역할 문서의 게임 특화 기준은 앱의 데이터 구조, 상태 전이, 이벤트 흐름, UI/UX, 예외 처리, 테스트 가능성, MVP 범위 명확성에 대응해 검토한다. 역할 문서를 적용할 수 없는 항목은 임의로 꾸미지 말고 남은 리스크로 반환한다.
...
- 심문관이 최신 구현 계획 문서를 `/grilling` 세션과 `/domain-modeling` 스킬로 검토하고 `종료 가능`으로 판정했다.
- 서브에이전트 A가 메인 에이전트 역할 문서 기준으로 구현 계획 문서를 직접 확인했다.
- 구현 계획 문서가 공식 스펙 문서의 요구사항을 누락 없이 충족한다.
- 구현 계획 문서에 공식 스펙과 모순되는 내용이 없다.
- 구현자가 바로 작업할 수 있을 정도로 구현 방향, 데이터 흐름, 상태 전이, 이벤트 흐름, UI/UX, 예외 처리, 테스트 기준이 명확하다.
- MVP 범위를 벗어나는 과설계가 없다.
- 스펙 확인 필요 항목, 보류 항목, 남은 리스크가 숨겨지지 않았다.

## 메인 에이전트에게 반환할 질문

- 구현 계획이 공식 스펙과 충돌해 사용자 직접 판단이 필요한 질문.
- 실제 `ccusage` 실행 없이는 확정할 수 없는 질문.
- 구현 범위 확장이 필요한 질문.
