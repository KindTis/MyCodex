...
MyCodex Usage Dashboard는 로컬 Codex 사용량을 작은 데스크톱 오버레이와 전체 대시보드로 보여주는 애플리케이션입니다. `ccusage`의 토큰·비용 데이터와 `codex app-server`의 5시간·1주 rate limit 및 리셋 시간을 하나의 화면에 모읍니다.
...
<p align="center">
  <img src="./docs/images/MyCodex_Usage_Overlay_20260711_211953978.png" alt="다음 리셋까지 남은 시간을 표시하는 MyCodex Windows 오버레이" />
</p>
...
- **상시 표시 오버레이** — 오늘 토큰, 오늘 비용, 5시간·1주 limit과 리셋 시간을 280 × 188 크기의 투명한 always-on-top 창에 표시합니다.
...
- **설정 저장** — 패널 불투명도, 자동 갱신 주기, 리셋 시간 표시 방식, 창 위치를 저장하고 다음 실행 시 복원합니다.
...
- 우클릭 후 **설정**을 선택하면 패널 불투명도, 갱신 주기, 리셋 시간 표시 방식을 변경할 수 있습니다.
- 기본 갱신 주기는 30초이며, 5초에서 300초 사이로 설정할 수 있습니다.
- 패널 불투명도는 20%에서 100% 사이로 설정할 수 있습니다.
- **Display reset as remaining time**을 켜면 리셋 시각 대신 다음 리셋까지 남은 시간을 `NEXT`로 표시합니다. 끄면 리셋 시각을 `RESET`으로 표시합니다.
